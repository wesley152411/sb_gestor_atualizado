...
export async function generateLogisticsPDF(eventData: PartyEvent): Promise<void> {
  ...
  doc.setFontSize(10);
  let y = 58;
  const dateFormatted = formatDate(eventData.event_date);

  const rows: [string, string, string, string][] = [
    ['Cliente:', eventData.client_name, 'Telefone:', eventData.phone],
    ['Tema:', eventData.theme, 'Valor:', `R$ ${Number(eventData.total_value).toFixed(2)}`],
  ...
  const eventDateOnly = eventData.event_date?.includes('T') ? eventData.event_date.split('T')[0] : eventData.event_date;
  const filename = `LOGISTICA_${eventData.client_name.replace(/\s+/g, '_')}_${eventDateOnly}.pdf`;
  doc.save(filename);
}
